...
export function ProfileMapGrid({ maps }: ProfileMapGridProps) {
  ...
  return (
    <motion.div
      ...
      className="px-4"
    >
      {/* Container that clips the cream background of the cards */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
          {maps.map((map) => (
            <motion.div
              ...
              className="relative bg-surface p-5 rounded-xl shadow-md flex flex-col items-center hover:shadow-lg transition-shadow"
            >
              <img
                src={map.image}
                alt={map.name}
                ...
                <div className="absolute inset-0 rounded-xl opacity-20 pointer-events-none">
                  <div className={
                    map.userRank === 1
                      ...
                      : map.userRank === 2
                        ? 'bg-gradient-to-br from-gray-300/30 to-gray-500/30'
